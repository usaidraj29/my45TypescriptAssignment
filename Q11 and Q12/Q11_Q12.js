// Store the names of a few of your friends in a array called names. Print each person’s name
var array = ["sufiyan", "asad", "rohit", "rehman"];
console.log(array[0]);
console.log(array[1]);
console.log(array[2]);
console.log(array[3]);
/*Start with the array you used in Exercise 11, but instead of just printing each
person’s name, print a message to them. The text of each message should be the same,*/
console.log("Have a Good Day ".concat(array[0]));
console.log("Have a Good Day ".concat(array[1]));
console.log("Have a Good Day ".concat(array[2]));
console.log("Have a Good Day ".concat(array[3]));
